...
def extract_surveys_data(file_path:str) -> pd.DataFrame:
    """
    Extracts airtime data from a excel file.

    Parameters:
    file_path (str): The path to the excel file containing airtime data.
    ...
    pattern = r"DATA\s+RESPON\s+PENDENGAR\s*(\d{4})"
    df_list=[]
    ...
    surveys_data = pd.concat(df_list, ignore_index=True)
    return surveys_data 